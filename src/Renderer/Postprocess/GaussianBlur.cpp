...
namespace Renderer
{
		...
		:m_readIndex(0), m_writeIndex(1), m_blurTimes(10)
	{
		...
		m_mergeShaderIndex = ShaderMgr::getSingleton()->loadShader("MergeBlur",
			ASSERTS_PATH "glsl/MergeBlur.vert", ASSERTS_PATH "glsl/MergeBlur.frag");
		m_gaussianShaderIndex = ShaderMgr::getSingleton()->loadShader("GaussianBlur",
			ASSERTS_PATH "glsl/GaussianBlur.vert", ASSERTS_PATH "glsl/GaussianBlur.frag");
	}
	...
	void GaussianBlur::renderGaussianBlurEffect()
	{
		...
		std::shared_ptr<Shader> blurShader = ShaderMgr::getSingleton()->getShader(m_gaussianShaderIndex);
		blurShader->bind();
		std::string texStr;
		...
		glClear(GL_COLOR_BUFFER_BIT);
		std::shared_ptr<Shader> mergeShader = ShaderMgr::getSingleton()->getShader(m_mergeShaderIndex);
