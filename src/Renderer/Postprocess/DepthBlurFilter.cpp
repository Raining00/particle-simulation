...
namespace Renderer
{
		...
		: DepthBlurFilter(w, h, screenMesh)
	{
		m_blurShaderIndex = ShaderMgr::getSingleton()->loadShader("DepthGaussianBlur",
			ASSERTS_PATH "glsl/DepthGaussianBlur.vert", ASSERTS_PATH "glsl/DepthGaussianBlur.frag");
	}
	...
	unsigned int DepthGaussianBlurFilter::blurTexture(unsigned int targetTexIndex, const glm::mat4 &projectMat)
	{
		...
		std::shared_ptr<Shader> blurShader = ShaderMgr::getSingleton()->getShader(m_blurShaderIndex);
		blurShader->bind();
		...
		:DepthBlurFilter(w, h, screenMesh)
	{
		m_blurShaderIndex = ShaderMgr::getSingleton()->loadShader("DepthBilateralSeperateBlur",
			ASSERTS_PATH "glsl/DepthBilateralSeperateBlur.vert", ASSERTS_PATH "glsl/DepthBilateralSeperateBlur.frag");
	}
		...
		: DepthBlurFilter(w, h, screenMesh)
	{
		m_blurShaderIndex = ShaderMgr::getSingleton()->loadShader("DepthBilateralCombineBlur",
			ASSERTS_PATH "glsl/DepthBilateralCombineBlur.vert", ASSERTS_PATH "glsl/DepthBilateralCombineBlur.frag");
	}
		...
		:DepthBlurFilter(w, h, screenMesh)
	{
		...
		m_blurShaderIndex = ShaderMgr::getSingleton()->loadShader("DepthCurvatureFlowBlur",
			ASSERTS_PATH "glsl/DepthCurvatureFlowBlur.vert", ASSERTS_PATH "glsl/DepthCurvatureFlowBlur.frag");
	}
