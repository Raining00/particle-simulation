...
namespace Renderer
{
		...
		:m_screenWidth(scrWidth), m_screenHeight(scrHei)
	{
		...
		m_shaderIndex = m_shaderMgr->loadShader("liquidDepth",
			ASSERTS_PATH "glsl/liquidDepth.vert", ASSERTS_PATH "glsl/liquidDepth.frag");
		m_shaderMgr->loadShader("liquidThick",
			ASSERTS_PATH "glsl/liquidThickness.vert", ASSERTS_PATH "glsl/liquidThickness.frag");
		m_shaderMgr->loadShader("liquidRender",
			ASSERTS_PATH "glsl/liquidRender.vert", ASSERTS_PATH "glsl/liquidRender.frag");
	...
	void LiquidDrawable::renderDepth(std::shared_ptr<Shader> shader, Camera3D::ptr lightCamera)
	{
		return;
	}
