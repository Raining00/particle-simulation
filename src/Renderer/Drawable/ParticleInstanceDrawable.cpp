...
namespace Renderer
{
	ParticleInstanceDrawable::ParticleInstanceDrawable(unsigned int shaderIndex)
	{
		m_shaderIndex = ShaderMgr::getSingleton()->loadShader("particleInstance",
			ASSERTS_PATH "glsl/particleInstance.vert", ASSERTS_PATH "glsl/particleInstance.frag");
		m_depthShaderIndex = ShaderMgr::getSingleton()->loadShader("particleInstanceDepth",
			ASSERTS_PATH "glsl/particleInstanceDepth.vert", ASSERTS_PATH "glsl/particleInstanceDepth.frag");
	}
	...
	void ParticleInstanceDrawable::renderDepth(std::shared_ptr<Shader> shader, Camera3D::ptr lightCamera)
	{
