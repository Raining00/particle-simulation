...
namespace Renderer
{
		...
		:m_posChannel(posChannel)
	{
		...
		m_shaderIndex = m_shaderMgr->loadShader("particle",
			ASSERTS_PATH "glsl/particle.vert", ASSERTS_PATH "glsl/particle.frag");
	...
	void ParticlePointSpriteDrawable::renderDepth(std::shared_ptr<Shader> shader, Camera3D::ptr lightCamera)
	{
		if (!m_visiable || !m_produceShadow)
			return;
		static ShaderMgr::ptr shaderMgr = ShaderMgr::getSingleton();
		static unsigned int index = shaderMgr->loadShader("spriteDepth", ASSERTS_PATH "glsl/spriteDepth.vert",
			ASSERTS_PATH "glsl/spriteDepth.frag");
