...
namespace Renderer
{
	bool Voxelization::voxelize(Drawable* target, const float & step, std::vector<glm::vec3>& ret)
	{
		...
		unsigned int voxelizeCount = shaderMgr->loadShader("voxelizeCount",
			ASSERTS_PATH "glsl/voxelizeCount.vert", ASSERTS_PATH "glsl/voxelizeCount.frag", ASSERTS_PATH "glsl/voxelizeCount.geom");
		std::shared_ptr<Shader> shader = shaderMgr->getShader(voxelizeCount);
